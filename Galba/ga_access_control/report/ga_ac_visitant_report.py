...
class attendance_print_visitant(report_sxw.rml_parse):
    def __init__(self, cr, uid, name, context):
        super(attendance_print_visitant, self).__init__(cr, uid, name, context=context)
        self.localcontext.update({
            'time': time,
            'lst': self._lst,
            'total': self._lst_total,
            'get_visitant': self._get_visitant,
        })

    def _get_visitant(self, visit_ids):
        visit_obj_list = self.pool.get('ga.ac.visitant').browse(self.cr, self.uid, visit_ids)
        return visit_obj_list

    def _lst(self, visitant_id, dt_from, dt_to, *args):
        #consult the records in the range
        self.cr.execute("select date_in, date_out, department_id, action, authorizes, observations_output_id, observations_entry_id, irregularity_output_id, irregularity_entry_id from hr_attendance where visitant_id=%s and to_char(name,'YYYY-mm-dd')<=%s and to_char(name,'YYYY-mm-dd')>=%s and action IN (%s,%s) order by name", (visitant_id, dt_to, dt_from, 'sign_in', 'sign_out'))
    ...
    def _lst_total(self, visitant_id, dt_from, dt_to, *args):
        self.cr.execute("select date_in, date_out, action from hr_attendance where visitant_id=%s and to_char(name,'YYYY-mm-dd')<=%s and to_char(name,'YYYY-mm-dd')>=%s and action IN (%s,%s) order by name", (visitant_id, dt_to, dt_from, 'sign_in', 'sign_out'))
        res = self.cr.dictfetchall()
        if not res:
            return ('/','/')
        average_in = []
...
class report_ga_attendance_visitant(osv.AbstractModel):
    _name = 'report.ga_access_control.report_attendance_visitant'
    _inherit = 'report.abstract_report'
    _template = 'ga_access_control.report_attendance_visitant'
    _wrapped_report_class = attendance_print_visitant
